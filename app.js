...
function chapterNumberFromTitle(title = "") {
  const match = String(title).match(/(?:chapter|ch\.?)\s*(\d+)/i) || String(title).match(/^(\d+)(?:\D|$)/);
  if (!match) return null;
  const value = Number.parseInt(match[1], 10);
  return Number.isFinite(value) && value > 0 ? value : null;
}

function inferNextChapterNumberForBook(book, entries = []) {
  if (!book?.key || !book.total) return null;
  const bookEntries = entries.filter(chapter => bookKey(chapter.bookTitle || "", chapter.authorName || "") === book.key);
  const usedNumbers = new Set(bookEntries.map(chapter => chapterNumberFromTitle(chapter.chapterTitle)).filter(Boolean));
  if (!usedNumbers.size && bookEntries.length < book.total) return bookEntries.length + 1;
  for (let number = 1; number <= book.total; number += 1) {
    if (!usedNumbers.has(number)) return number;
  }
  return null;
}

function autoDraftIdForBook(book, chapterNumber) {
  return `auto-draft::${book.key}::chapter-${chapterNumber}`;
}

function dismissedAutoDraftIds() {
  return new Set(safeReadStorage(DISMISSED_AUTO_DRAFTS_KEY, []) || []);
}

function dismissAutoDraft(id = "") {
  if (!id) return;
  safeWriteStorage(DISMISSED_AUTO_DRAFTS_KEY, [...dismissedAutoDraftIds(), id]);
}

function upNextDraftForBook(bookTitle = "", authorName = "", entries = loadChapters()) {
  const key = bookKey(bookTitle, authorName || "");
  return entries
    .filter(chapter => chapter.status === "Draft" && bookKey(chapter.bookTitle || "", chapter.authorName || "") === key)
    .sort((a, b) => {
      const aAuto = a.autoGenerated ? 1 : 0;
      const bAuto = b.autoGenerated ? 1 : 0;
      if (aAuto !== bAuto) return aAuto - bAuto;
      return new Date(b.updatedAt || b.createdAt || 0) - new Date(a.updatedAt || a.createdAt || 0);
    })[0] || null;
}

function ensureUpNextChapterDrafts(entries = loadChapters()) {
  if (!isLoggedIn()) return entries;
  const books = loadBookMetadata();
  const groups = new Map();
  const dismissedDrafts = dismissedAutoDraftIds();

  entries.forEach(chapter => {
    const book = bookRecordForChapter(chapter, books);
    if (!book.key || !book.total) return;
    if (!groups.has(book.key)) groups.set(book.key, { book, entries: [] });
    groups.get(book.key).entries.push(chapter);
  });

  books.forEach(metadata => {
    const key = metadata.key || bookKey(metadata.title || "", metadata.author || "");
    const total = Number.parseInt(metadata.totalChapters || "", 10);
    if (!key || !Number.isFinite(total) || total <= 0 || groups.has(key)) return;
    groups.set(key, {
      book: {
        key,
  ...
  groups.forEach(group => {
    const completedCount = group.entries.filter(chapter => chapter.status !== "Draft").length;
    const hasDraft = group.entries.some(chapter => chapter.status === "Draft");
    if (hasDraft || completedCount >= group.book.total) return;
    const chapterNumber = inferNextChapterNumberForBook(group.book, entries);
    if (!chapterNumber) return;
    const id = autoDraftIdForBook(group.book, chapterNumber);
    if (dismissedDrafts.has(id)) return;
    if (entries.some(chapter => chapter.id === id)) return;
    const now = new Date().toISOString();
    draftsToAdd.push({
      id,
      bookPath: "existing",
      bookTitle: group.book.title,
      authorName: group.book.author || "",
      bookTotalChapters: String(group.book.total),
      ...
      status: "Draft",
      autoGenerated: true,
      autoGeneratedReason: "up-next-chapter",
      draftStep: 0,
      draftIntakeStep: 0,
      createdAt: now,
      updatedAt: now
  ...
  if (!draftsToAdd.length) return entries;
  const nextEntries = [...entries, ...draftsToAdd];
  saveChapters(nextEntries);
  return nextEntries;
}
...
function currentOpenDraft() {
  if (!state.currentId) return null;
  return loadChapters().find(chapter => chapter.id === state.currentId && chapter.status === "Draft") || null;
}

function syncDraftActions() {
  const button = $("#delete-entry-draft");
  if (!button) return;
  const draft = currentOpenDraft();
  button.hidden = !draft;
...
function deleteCurrentDraft() {
  const draft = currentOpenDraft();
  if (!draft) {
    toast("No saved draft to delete.");
    syncDraftActions();
    return;
  }
  openDeleteDraftDialog(draft);
}

function openDeleteDraftDialog(draft) {
  const dialog = $("#delete-draft-dialog");
  if (!dialog || !draft) return;
  state.pendingDeleteDraftId = draft.id;
  $("#delete-draft-copy").textContent = `${draft.chapterTitle || "This chapter"} will be removed from your drafts. You can start it again later.`;
  dialog.showModal();
  $("#confirm-delete-draft")?.focus();
}

function closeDeleteDraftDialog() {
  const dialog = $("#delete-draft-dialog");
  if (dialog?.open) dialog.close();
  state.pendingDeleteDraftId = null;
}

function confirmDeleteDraft() {
  const draft = loadChapters().find(chapter => chapter.id === state.pendingDeleteDraftId && chapter.status === "Draft");
  if (!draft) {
  ...
  if (draft.autoGenerated) dismissAutoDraft(draft.id);
  saveChapters(loadChapters().filter(chapter => chapter.id !== draft.id));
  try {
    const savedDraft = JSON.parse(localStorage.getItem(DRAFT_KEY));
    if (!savedDraft?.id || savedDraft.id === draft.id) localStorage.removeItem(DRAFT_KEY);
  } catch {
...
function buildAdaptiveNextAction({ evidenceState, entries, chapters, drafts, due, currentReading, practiceState }) {
  ...
  if (drafts.length) {
    ...
    return {
      ...
      copy: "You already started this chapter. Finish writing your review before moving on.",
      text: "Resume draft",
...
function renderSkillIcon(skillId = "central-claim", stateName = "unexplored", size = 32) {
  ...
  const icons = {
    "retrieve-explicit": `<svg ${common}><path class="skill-icon-noise" d="M5.8 9.3c1.8-1.2 3.3-1.1 4.4.2M13.7 6.8c1.9-.2 3.4.5 4.5 2.2M6.4 15.5c1.4.8 2.8.8 4.3-.1M13.4 17.3c1.7.1 3.1-.6 4.1-2"/><path class="skill-icon-operation" d="M8.3 8.6 12 12m3.7-3.4L12 12m0 0-3.8 3.5M12 12l3.9 3.4"/><circle class="skill-icon-foundation" cx="8.3" cy="8.6" r="1.9"/><circle class="skill-icon-foundation" cx="15.7" cy="8.6" r="1.9"/><circle class="skill-icon-foundation" cx="8.2" cy="15.6" r="1.9"/><circle class="skill-icon-foundation" cx="15.9" cy="15.4" r="1.9"/><path class="skill-icon-accent skill-icon-fill" d="M12 9.8 14 12l-2 2.2L10 12z"/></svg>`,
    "central-claim": `<svg ${common}><path class="skill-icon-foundation" d="M5.5 12c0-3.8 2.7-6.6 6.5-6.6s6.5 2.8 6.5 6.6-2.7 6.6-6.5 6.6S5.5 15.8 5.5 12Z"/><path class="skill-icon-operation" d="M8.6 12c0-2 1.4-3.5 3.4-3.5s3.4 1.5 3.4 3.5-1.4 3.5-3.4 3.5S8.6 14 8.6 12Z"/><path class="skill-icon-accent skill-icon-fill" d="m12 9.8 1.9 2.2-1.9 2.2-1.9-2.2z"/></svg>`,
    "supporting-ideas": `<svg ${common}><path class="skill-icon-foundation" d="M6.7 16.5h10.6M8.2 12.6h7.6M9.9 8.7h4.2"/><path class="skill-icon-operation" d="m12 5.2 5.3 11.3H6.7Z"/><circle class="skill-icon-accent skill-icon-fill" cx="12" cy="8.7" r="1"/><circle class="skill-icon-accent skill-icon-fill" cx="8.2" cy="16.5" r=".9"/><circle class="skill-icon-accent skill-icon-fill" cx="15.8" cy="16.5" r=".9"/></svg>`,
    "source-fidelity": `<svg ${common}><path class="skill-icon-foundation" d="M7.2 5.8h8.7l1.9 2v10.4H7.2z"/><path class="skill-icon-operation" d="M15.9 5.8v3h3M9.4 10.4h5.2M9.4 13.2h4.2"/><path class="skill-icon-accent" d="m9.2 16.1 1.2 1.1 2.5-3"/></svg>`,
    "connect-ideas": `<svg ${common}><path class="skill-icon-foundation" d="M6.5 12c0-2 1.5-3.5 3.5-3.5h1.2M12.8 8.5H14c2 0 3.5 1.5 3.5 3.5S16 15.5 14 15.5h-1.2M11.2 15.5H10c-2 0-3.5-1.5-3.5-3.5"/><path class="skill-icon-operation" d="M9.9 12h4.2"/><circle class="skill-icon-accent skill-icon-fill" cx="8.1" cy="12" r="1"/><circle class="skill-icon-accent skill-icon-fill" cx="15.9" cy="12" r="1"/></svg>`,
    "structure": `<svg ${common}><path class="skill-icon-foundation" d="M7.1 6.5h9.8v3.7H7.1zM7.1 13.8h9.8v3.7H7.1z"/><path class="skill-icon-operation" d="M12 10.2v3.6M9.4 12h5.2"/><path class="skill-icon-accent" d="M9.4 8.3h5.2M10 15.6h4"/></svg>`,
    "point-of-view": `<svg ${common}><path class="skill-icon-foundation" d="M5.1 12s2.7-4.9 6.9-4.9 6.9 4.9 6.9 4.9-2.7 4.9-6.9 4.9S5.1 12 5.1 12Z"/><path class="skill-icon-operation" d="M10 12a2 2 0 1 0 4 0 2 2 0 0 0-4 0Z"/><path class="skill-icon-accent" d="M15.8 7.3 18 5.6M8.2 16.7 6 18.4"/><circle class="skill-icon-accent skill-icon-fill" cx="12.7" cy="11.2" r=".8"/></svg>`,
    "match-evidence": `<svg ${common}><path class="skill-icon-foundation" d="M6.3 7.4h5.4v5.4H6.3zM12.3 11.2h5.4v5.4h-5.4z"/><path class="skill-icon-operation" d="m8.1 10 1.1 1 2.3-2.5M14.1 13.8l1.1 1 2.3-2.5"/><path class="skill-icon-accent" d="M10.6 12.1 13.1 14"/></svg>`,
    "evaluate-evidence": `<svg ${common}><path class="skill-icon-foundation" d="M6.4 6.2h11.2v11.6H6.4z"/><path class="skill-icon-noise" d="M8.7 9.2h6.6M8.7 12h4.2"/><path class="skill-icon-accent" d="m8.7 15.2 1.6 1.5 4.9-5.5"/></svg>`,
    "evaluate-reasoning": `<svg ${common}><path class="skill-icon-foundation" d="M12 5.6v12.2M7.2 8.5h9.6"/><path class="skill-icon-operation" d="m7.2 8.5-2.1 4h4.2zM16.8 8.5l-2.1 4h4.2z"/><path class="skill-icon-accent" d="M8.5 17.8h7"/><circle class="skill-icon-accent skill-icon-fill" cx="12" cy="8.5" r=".95"/></svg>`,
    "build-explanation": `<svg ${common}><path class="skill-icon-foundation" d="M6.1 17.2h11.8M7.8 13.6h8.4M9.6 10h4.8"/><path class="skill-icon-operation" d="m12 5.4 3.1 4.6H8.9Z"/><path class="skill-icon-accent" d="M12 10v7.2"/><circle class="skill-icon-accent skill-icon-fill" cx="12" cy="13.6" r="1"/></svg>`,
    "infer-implications": `<svg ${common}><path class="skill-icon-foundation" d="M6.8 17.2 17.2 6.8"/><path class="skill-icon-operation" d="M11.2 6.8h6v6"/><path class="skill-icon-noise" d="M7 10.2c1.3-.8 2.5-.9 3.6-.2M13.4 17.1c1.2.2 2.3-.1 3.3-.9"/><circle class="skill-icon-accent skill-icon-fill" cx="7" cy="17" r="1.3"/><circle class="skill-icon-accent skill-icon-fill" cx="17.2" cy="6.8" r="1.3"/></svg>`,
    "compare-texts": `<svg ${common}><path class="skill-icon-foundation" d="M5.8 6.5h6.4v10.8H5.8zM11.8 6.8h6.4v10.5h-6.4z"/><path class="skill-icon-operation" d="M8.1 10h2.4M14.1 10h1.8M8.1 13.5h2.4M14.1 13.5h1.8"/><path class="skill-icon-accent" d="M12 8v8"/></svg>`,
    "clear-explanations": `<svg ${common}><path class="skill-icon-foundation" d="M5.5 7.4h13v7.9h-4.8L10.2 18l.7-2.7H5.5z"/><path class="skill-icon-operation" d="M8.6 10.1h6.8M8.6 12.7h4.7"/><path class="skill-icon-accent" d="m16.7 5.6.7 1.5 1.5.7-1.5.7-.7 1.5-.7-1.5-1.5-.7 1.5-.7z"/></svg>`,
    "calibrate-confidence": `<svg ${common}><path class="skill-icon-foundation" d="M6 15.4a6 6 0 1 1 12 0"/><path class="skill-icon-noise" d="M8.1 15.2h7.8"/><path class="skill-icon-operation" d="M12 14.2 15.4 9"/><path class="skill-icon-accent" d="M7.8 11.4h1.4M14.8 7.9l1 1"/><circle class="skill-icon-accent skill-icon-fill" cx="12" cy="14.2" r="1.2"/></svg>`,
    "apply-with-judgment": `<svg ${common}><path class="skill-icon-foundation" d="m12 4.8 2.3 4.7 5.2.8-3.8 3.7.9 5.2-4.6-2.4-4.6 2.4.9-5.2-3.8-3.7 5.2-.8z"/><path class="skill-icon-operation" d="m9.4 12.5 1.6 1.5 3.7-4.1"/><path class="skill-icon-accent" d="M12 4.8v3.3M16.6 19.2l-2.1-2"/></svg>`,
    "evaluate-boundaries": `<svg ${common}><path class="skill-icon-foundation" d="M6.2 12c0-3.3 2.5-5.8 5.8-5.8s5.8 2.5 5.8 5.8-2.5 5.8-5.8 5.8S6.2 15.3 6.2 12Z"/><path class="skill-icon-operation" d="M12 6.2v11.6"/><path class="skill-icon-noise" d="M8.3 8.3 15.7 15.7"/><path class="skill-icon-accent" d="M8.2 12h7.6"/></svg>`
  };
...
const skillMapEdges = [
  ...
  { from: "point-of-view", to: "evaluate-reasoning", type: "supports", reason: "Author purpose shapes which claims are emphasized and how evidence is framed." },
  ...
  { from: "evaluate-reasoning", to: "compare-texts", type: "extends", reason: "Once readers can test one argument, they can compare how another text handles the same idea." },
...
const skillMapRegionSkillIds = {
  recall: ["retrieve-explicit", "central-claim", "source-fidelity"],
  meaning: ["supporting-ideas", "connect-ideas", "structure", "infer-implications"],
  judgment: ["point-of-view", "match-evidence", "evaluate-reasoning", "compare-texts"],
  transfer: ["build-explanation", "apply-with-judgment", "evaluate-boundaries", "calibrate-confidence"]
};
...
function skillMapRegionForSkill(skillId) {
  return Object.entries(skillMapRegionSkillIds).find(([, ids]) => ids.includes(skillId))?.[0] || "meaning";
}

function skillMapPathwaysForSkill(skillId) {
  return skillMapPathways.filter(pathway => pathway.skills.includes(skillId));
}

function skillMapPrimaryPathwayForSkill(skillId) {
...
function renderSkillMapPage() {
  ...
  const edges = skillMapEdges.map(edge => {
    ...
    return `<line class="relationship-${escapeHtml(edge.type)} edge-region-${escapeHtml(edgeRegion)}${active ? " is-active" : ""}${complete ? " is-complete" : ""}${locked ? " is-locked" : ""}" x1="${line.x1}" y1="${line.y1}" x2="${line.x2}" y2="${line.y2}">
...
document.addEventListener("click", async event => {
  ...
  if (action === "home-start-next-chapter") {
    ...
    const draft = upNextDraftForBook(button?.dataset.bookTitle || "", button?.dataset.authorName || "");
    if (draft) {
      openChapter(draft.id);
      return;
    }
    startNew({
      bookPath: "existing",
  ...
  if (action === "close-delete-draft") closeDeleteDraftDialog();
  ...
  if (action === "skill-map-select") {
    ...
    state.selectedSkillMapRegion = "0";
    renderSkillMapPage();
  }
...
$("#ember-guide-minimize")?.addEventListener("click", event => {
  event.stopPropagation();
  minimizeEmberCompanion();
});
$("#ember-companion-minimized")?.addEventListener("click", event => {
  event.stopPropagation();
  restoreEmberCompanion();
});
...
$("#delete-draft-dialog")?.addEventListener("click", event => {
  if (event.target === $("#delete-draft-dialog")) closeDeleteDraftDialog();
});
$("#delete-draft-dialog")?.addEventListener("close", () => {
  state.pendingDeleteDraftId = null;
});
